...
public class StagePlay extends Stage {
  ...
  private final Sound music = Gdx.audio.newSound(Gdx.files.internal("music/Bomno.wav"));
  ...
  private final StageChange stageChangeLose;
  private final StageChange stageChangeWin;
  ...
  public void draw() {
    ...
    if (chuyenman == false) {
      ...
      if (lv < 4) {
        ...
        // cheat qua man
        if ((stageScreens.get(lv - 1).CheckAllEnemyDeath()
                && stageScreens.get(lv - 1).CheckInPortal())
